...
APPLY_DTBO_PATCH() {
    local PATCH="$SRC_DIR/target/a53x/patches/dtbo/patches/$1"
...
PACK_TO_DTBO() {
    ...
    for i in "${!REVS[@]}"; do
        EVAL "dtc -I dts -O dtb -o $TMP_DIR/a53x_eur_open_w00_r0${REVS[$i]}.dtbo $TMP_DIR/dtsi.$i"
    done
}

PACK_TO_IMG() {
    local CONF="$SRC_DIR/target/a53x/patches/dtbo/a53x.cfg"
